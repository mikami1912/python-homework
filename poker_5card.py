...
#   input = ['2H', '3C', '4D', '5C', '1D' ] --> [14]  (1-5)
    hand = split_hand(input_hand)
    card_type = [int(item) for item in hand[0]]
    if (max(card_type)-min(card_type) == 4) and len(set(card_type)) == 5: # check if straight except 1-5 
        if max(card_type, key=abs) == 14: return 15
        else: return max(card_type, key=abs)
    elif sorted(card_type) == [2,3,4,5,14] : return 14
    else: return None
...
def test_list(number_of_hand): #return random list of number_of_hand to test
    ...
    for i in range(0,number_of_hand-1):
        a_hand = random.sample(deck, 5)
        for j in a_hand: deck.remove(j)
        output_list.append(a_hand)
    return output_list

def test():
    deck = initialize_deck()
    #t1 = random.sample(deck,5)
    t2 = ['3S', '1D', '1S', 'TC', '4C']
    #hand = split_hand(t1)
    #print(hand)
    #print(max([int(i) for i in hand[0]], key=abs)) #test high card: OK
    #print(straight(t2)) #test straight : OK
